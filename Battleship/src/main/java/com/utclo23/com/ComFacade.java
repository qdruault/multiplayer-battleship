...
public class ComFacade {
    ...
    // envoi au dest
    public void sendShipsToEnnemy(List<Ship> listShips, PublicUser dest) throws UnknownHostException{
        M_PlaceShip m_placeship = new M_PlaceShip(iDataCom, listShips);
    ...
    // envoi à tout le monde
    public void notifyUserSignedOut(PublicUser user) throws UnknownHostException{
        M_Deconnexion m_deconnexion = new M_Deconnexion(iDataCom, user);
        for(Inet4Address ip : kIpCtrl.getHashMap().values()){
    ...
    public void notifyNewMessage(com.utclo23.data.structure.Message message) throws UnknownHostException {
        M_Chat m_chat = new M_Chat(iDataCom, message, message.getTimestamp());
    ...
    public void notifyNewCoordinates(Mine mine, List<LightPublicUser> recipients) throws UnknownHostException{
        M_PlaceMine m_placemine = new M_PlaceMine(iDataCom, mine);
    ...
    // à tout le monde
    public void notifyNewGame(StatGame game) throws UnknownHostException{
        M_CreationGame m_creationgame = new M_CreationGame(iDataCom, game);
        for(Inet4Address ip : kIpCtrl.getHashMap().values()){
    ...
    public void connectionToGame(StatGame game) throws UnknownHostException{
        M_JoinGame m_joingame = new M_JoinGame(iDataCom, game);
    ...
    public void leaveGame(PublicUser user) throws UnknownHostException{
        M_LeaveGame m_leavegame = new M_LeaveGame(iDataCom);
        for(Inet4Address ip : kIpCtrl.getHashMap().values()){
    ...
    // envoi à tout le monde
    public void sendDiscovery(PublicUser user, List<Inet4Address> listIpTarget) throws UnknownHostException {
        
        for (int i = 0; i < listIpTarget.size(); i++) {
            M_GetIP m_getIp = new M_GetIP(iDataCom);
            m_getIp.setUser(user);
    ...
    // envoi à l'id
    public void getPublicUserProfile(String id) throws UnknownHostException{
        M_GetPlayerInfo m_getplayerinfo = new M_GetPlayerInfo(iDataCom);
        Sender os = new Sender(kIpCtrl.getHashMap().get(id).getHostAddress(), 80, m_getplayerinfo);
        new Thread(os).start();
    }

    // envoi à tout le monde
    public void joinGameResponse (boolean success, String id, StatGame game) throws UnknownHostException{
        M_JoinGameResponse m_joingameresponse = new M_JoinGameResponse(iDataCom, success);
        for(Inet4Address ip : kIpCtrl.getHashMap().values()){
